"""
Hardmine — TDM Cotton Exports Ingest
======================================
Usage:
    python cotton_exports_ingest.py            # incremental
    python cotton_exports_ingest.py --full     # full history from 201701

Saves to: ../Database/tdm_cotton_exports.parquet
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "cotton_exports_ingest.log", encoding="utf-8"),
    ],
)
...
REPORTERS = "BR,US,CI,AR,CN,TW,PK,SG,KR,NZ,UA,TR,AU"
...
OUT_FILE = Path(__file__).parents[1] / "Database" / "tdm_cotton_exports.parquet"
...
def fetch_tdm(period_begin: str) -> pd.DataFrame:
    ...
    log.info("Fetching Cotton Exports from %s ...", period_begin)
...
def main():
    parser = argparse.ArgumentParser(description="Cotton Exports Ingest")
    parser.add_argument("--full", action="store_true")
    args = parser.parse_args()

    log.info("=" * 60)
    log.info("Cotton Exports Ingest  |  %s", datetime.now().strftime("%Y-%m-%d %H:%M"))
