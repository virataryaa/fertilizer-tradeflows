"""
Hardmine — TDM Fertilizer Exports Ingest (HS Chapter 31)
==========================================================
Usage:
    python fertilizer_exports_ingest.py            # incremental
    python fertilizer_exports_ingest.py --full     # full history from 202001

Saves to: ../Database/tdm_fertilizer_exports.parquet
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / "fertilizer_exports_ingest.log", encoding="utf-8"),
    ],
)
...
# Top fertilizer exporters (Nigeria excluded — partner-only in TDM)
REPORTERS = "CN,MA,CA,SA,US,NL,BE,QA,EG,ESC,IR,IL,JO,DEC,PLC,RU"
...
def fetch_tdm(period_begin: str) -> pd.DataFrame:
    ...
    log.info("Fetching Fertilizer Exports from %s ...", period_begin)
...
def main():
    parser = argparse.ArgumentParser(description="Fertilizer Exports Ingest")
    parser.add_argument("--full", action="store_true", help="Full pull from PERIOD_FULL_BEGIN")
    args = parser.parse_args()

    log.info("=" * 60)
    log.info("Fertilizer Exports Ingest  |  %s", datetime.now().strftime("%Y-%m-%d %H:%M"))
